...
mod tests {
    ...
    use sncast::helpers::scarb_utils::{get_package_metadata, CastConfig};
    use std::fs;
    ...
    fn test_add_created_profile_to_configuration_happy_case() {
        ...
        let package = get_package_metadata(&Utf8PathBuf::from("Scarb.toml"), &None).unwrap();
        let res = add_created_profile_to_configuration(&package, &config);
    ...
    fn test_add_created_profile_to_configuration_profile_already_exists() {
        ...
        let package = get_package_metadata(
            &Utf8PathBuf::from("Scarb.toml"),
            &Some("constructor_with_params".into()),
        )
        .unwrap();
        let res = add_created_profile_to_configuration(&package, &config);
